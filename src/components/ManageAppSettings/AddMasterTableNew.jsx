...
const AddMasterTableNew = React.memo((props) => {
  ...
  const tableDetailsListById = useSelector(
  ...
  const selectedModuleList =
    tableDetailsListById.moduleTableConfigModules &&
    tableDetailsListById.moduleTableConfigModules.map((id) => id.module.id);
  ...
  const [parentTableNameValue, setParentTableNameValue] = useState(false);
  ...
  const handleSave = (tableVal, fieldName) => {
    setParentTableNameValue(fieldName)
    parentFieldName.push(fieldName)
    setParentSelection(tableVal, fieldName)
    setOpenParentDialog(false);
  }
  const openParentPopup = (fieldName) => {
    setChildFieldName(fieldName)
  ...
  const defaultFormData = {
    ...
    moduleIds:
      selectedModuleList?.length > 0
        ? selectedModuleList.filter(
          (value, index) => selectedModuleList.indexOf(value) === index
        )
        : [],
    subText: "",
    tableLabel: tableDetailsListById.tableLabel,
    childField:
      tableDetailsListById.childFieldList?.length > 0
        ? tableDetailsListById.childFieldList
        : [],
  };
  const [inputs, setInputs] = useState(defaultFormData);
  const colName = useSelector((state) => state.MasterTable.columnList);
  const newMapField = JSON.parse(tableDetailsListById.mapField) || [];

  let newMap = newMapField
    .filter((item) => item.mapObject?.isRequired)
    .map((item) => item.mapObject?.fieldName);

  let getFields = colName?.filter(
    (item) => item.isRequired && newMap.indexOf(item?.fieldName) === -1
  );

  let setFields = getFields?.map((data) => {
    let blankData = {
      limit: {},
      mapLable: data?.columnName === "id" ? "id" : "",
      mapReference: false,
      parentName: "",
  ...
  if (setFields.length > 0 || newMapField.length > 0) {
    ...
    defval =
      mapField && Object.keys(mapField).length !== 0
        ? mapField.map((obj) => obj.mapObject.fieldName)
        : [];
  ...
  const { tableName, moduleIds, subText, tableLabel, childField } = inputs;
  ...
  const [primaryKey, setPrimaryKey] = useState(
    filterObjects &&
      ...
      filterObjects[0].mapObject?.fieldName
      ? filterObjects[0].mapObject?.fieldName
      : ""
  );
  ...
  let handleLabelChange = (e, val) => {
    ...
    dispatch({ type: RESET_UPDATE_ERROR });
    let label = payload
      .filter((obj) => obj.mapObject?.fieldName !== val)
      .map((item) => item.mapLable.toLowerCase());
    if (name === "subText") {
      if (label.indexOf(value.toLowerCase()) !== -1) setIsLabelUpdated(true);
      else setIsLabelUpdated(false);
    }
    setInputs((inputs) => ({ ...inputs, [name]: value }));
  };
  ...
  const handleCheckChange = (value) => {
    ...
    if (currentIndex === -1) {
      ...
      if (obj?.isAutoIncrement === "true") setAutoKey(value);
      newPayload.push(Json);
    } else {
      var removeIndex = newPayload
        .map((item) => item.mapObject?.fieldName)
        .indexOf(value);
  ...
  //       setInputs((inputs) => ({
  //         ...inputs,
  //         childField: defaultFormData.childField,
  //       }));
  //     else setInputs((inputs) => ({ ...inputs, childField: value }));
  //   };

  const setParentSelection = (parentName, fieldName) => {
    let newPayload = payload.map((item) => {
      if (item.mapObject?.fieldName === fieldName) {
        let newItem;
        newItem = { ...item, parentName: parentName, defaultVal: "", limit: {} };
        return newItem;
      } else {
        return { ...item };
      }
  ...
  const handleUpdatePayload = (updatedData) => {
    if (Boolean(updatedData.defaultVal)) {
      if (Boolean(updatedData.limit.min) || Boolean(updatedData.limit.max)) {
        if (updatedData.fieldType === "Calendar Field") {
          if (
            (Boolean(updatedData.limit.min) &&
              !Boolean(updatedData.limit.max) &&
              new Date(updatedData.defaultVal) >=
              new Date(updatedData.limit.min)) ||
            (Boolean(updatedData.limit.max) &&
              !Boolean(updatedData.limit.min) &&
              new Date(updatedData.defaultVal) <=
              new Date(updatedData.limit.max)) ||
            (Boolean(updatedData.limit.min) &&
              Boolean(updatedData.limit.max) &&
              new Date(updatedData.defaultVal) >=
              new Date(updatedData.limit.min) &&
            ...
            if (defaultError.indexOf(updatedData.mapObject?.fieldName) !== -1) {
              let Index = defaultError.indexOf(
                updatedData.mapObject?.fieldName
              );
              defaultError.splice(Index, 1);
            }
          } else {
        ...
        if (updatedData.fieldType === "Text Field") {
          if (
            (Boolean(updatedData.limit.min) &&
              !Boolean(updatedData.limit.max) &&
              updatedData.defaultVal.length >=
              parseInt(updatedData.limit.min)) ||
            (Boolean(updatedData.limit.max) &&
              !Boolean(updatedData.limit.min) &&
              updatedData.defaultVal.length <=
              parseInt(updatedData.limit.max)) ||
            (Boolean(updatedData.limit.min) &&
              Boolean(updatedData.limit.max) &&
              updatedData.defaultVal.length >=
              parseInt(updatedData.limit.min) &&
        ...
        if (
          updatedData.fieldType === "Integer Field" &&
          updatedData.mapObject.javaDataType === "DOUBLE"
        ) {
          if (
            (Boolean(updatedData.limit.min) &&
              !Boolean(updatedData.limit.max) &&
              parseInt(updatedData.defaultVal) >=
              parseInt(updatedData.limit.min)) ||
            (Boolean(updatedData.limit.max) &&
              !Boolean(updatedData.limit.min) &&
              parseInt(updatedData.defaultVal) <=
        ...
        if (
          updatedData.fieldType === "Integer Field" &&
          updatedData.mapObject.javaDataType === "INTEGER"
        ) {
          if (
            (Boolean(updatedData.limit.min) &&
              !Boolean(updatedData.limit.max) &&
            ...
            if (
              defaultError.indexOf(updatedData.mapObject?.fieldName) === -1 ||
              isDecimal(updatedData.defaultVal)
            )
              defaultError.push(updatedData.mapObject?.fieldName);
          }
    ...
    let newPayload = payload.map((item) => {
      if (item.mapObject?.fieldName === updatedData.mapObject?.fieldName) {
        ...
        newItem = {
          ...item,
          defaultVal: updatedData.defaultVal,
  ...
  const handleTitleMsg = (value) => {
    return payload.filter((item) => item.mapObject?.fieldName === value)
      .length > 0 &&
      payload.filter((item) => item.mapObject?.fieldName === value)[0]
        .mapLable === ""
      ? "Set Label"
      : "Edit Label";
  };
  ...
  useEffect(() => {
    if (
      ...
      setApiError(
        handleUpdateMasterTableError(putApiError, defaultFormData.tableName)
      );
    } else {
      setApiError(false);
      setIsSubmited(false);
    ...
    isTableUpdated,
    handleCloseForm,
    defaultFormData.tableName,
  ]);

  const checkValidation = () => {
    let isValid = true;
    let validationText = [];
    let isPrimary = payload.find((item) => item.mapReference);
    if (!isPrimary) {
      validationText.push("One primary field is required");
      isValid = false;
    }
    for (let i = 0; i < payload.length; i++) {
      if (payload[i].mapLable === "") {
        validationText.push(
          `Column name "${payload[i].mapObject.columnName}" label value is required`
        );
        isValid = false;
      }
      if (payload[i].fieldType === "") {
        validationText.push(
          `Column name "${payload[i].mapObject.columnName}" field type is required`
        );
        isValid = false;
      }
      if (defaultError.indexOf(payload[i].mapObject.columnName) !== -1) {
        validationText.push(
          `Column name "${payload[i].mapObject.columnName}" default value must be within set limits`
        );
        isValid = false;
      }
    }
  ...
  const handleUpdateTable = () => {
    ...
    let modifiedPayload = payload.filter((a, i) => {
      if (a.fieldType === "Integer Field") {
        payload[i]["defaultVal"] = payload[i]["defaultVal"] === "" ? 0 : parseInt(payload[i]["defaultVal"])
      }
  ...
  useEffect(() => {
    if (tableName !== "") dispatch(fetchColumnList(tableName));
  }, [dispatch, tableName]);
  ...
  return (
    <Dialog
      ...
      maxWidth="lg"
    >
      <DialogTitle className={styles.dialogTitle}>{UPDATE_TABLE}</DialogTitle>
      <form
        noValidate
        autoComplete="off"
      ...
      >
                      ...
                      {colName.map((option, key) => (
                        <TableRow key={option.fieldName}>
                          <TableCell>
                            <Checkbox
                              onChange={() =>
                                handlePrimaryChange(option.fieldName)
                              }
                              disabled={
                                ...
                                autoKey === option.fieldName ||
                                option.fieldName === "id"
                              }
                              checked={
                                primaryKey === option.fieldName && key !== 0
                              }
                            />
                          </TableCell>
                          <TableCell>
                            <Checkbox
                              onChange={() =>
                                handleCheckChange(option.fieldName)
                              }
                              disabled={
                                (option.fieldName === "version" &&
                                  option.isRequired &&
                            ...
                            {editable === option?.fieldName ? (
                              <TextField
                                ...
                                defaultValue={
                                  payload.filter(
                                    (item) =>
                                      item.mapObject?.fieldName ===
                                      option.fieldName
                                  )[0]?.mapLable
                                }
                                onChange={(e) =>
                                  handleLabelChange(e, option.fieldName)
                                }
                              />
                            ) : payload.filter(
                              (item) =>
                                item.mapObject?.fieldName === option.fieldName
                            ).length > 0 &&
                              payload.filter(
                                (item) =>
                                  item.mapObject?.fieldName === option.fieldName
                          ...
                          <TableCell>
                            {editable === option.fieldName ? (
                              <>
                                <Tooltip placement="left" arrow title="Save">
                                  <IconButton
                                    ...
                                    onClick={() =>
                                      handleSetSubText(option.fieldName)
                                    }
                                  >
                                    <CheckIcon />
                                  </IconButton>
                              ...
                              <Tooltip
                                placement="left"
                                arrow
                                title={handleTitleMsg(option.fieldName)}
                              >
                                <IconButton
                                  disabled={
                                    option.fieldName === "id" ||
                                  ...
                                  aria-label="comments"
                                  onClick={() =>
                                    handleSetLabel(option.fieldName)
                                  }
                                >
                                  <CommentIcon />
                                </IconButton>
                          ...
                          <TableCell>
                            <IconButton
                              onClick={() => openParentPopup(option.fieldName)}
                              disabled={option.parentTableName ? true : false ||
                                (payload.filter(
                                  (item) =>
                                    item.mapObject?.fieldName ===
                                    option.fieldName
                                ).length > 0) ||
                                editable === option.fieldName ? false : true}>
                              <ArrowDropDownCircleIcon
                                style={{
                                  color:
                                    (Array?.isArray(parentFieldName) && parentFieldName?.indexOf(option.fieldName) !== -1) ?
                                      "#4ADAB3" : "",
                                }}
                          ...
                          <TableCell>
                            {checked?.includes(option.fieldName) && (
                              <TextField
                                ...
                                onChange={(event) => {
                                  handleFieldSelection(event, option.fieldName);
                                }}
                                fullWidth={true}
                              >
                                {/* 10>9?10>8?1:2 */}
                                {FEILD_TYPE.filter(
                                  (item) => item.id === option.javaDataType
                                ).length > 0
                                  ? FEILD_TYPE.filter(
                                    (item) => item.id === option.dataType
                                  ).length > 0
                                    ? FEILD_TYPE.filter(
                                      (item) => item.id === option.dataType
                                    ).map((_option, index) => (
                                    ...
                                    : FEILD_TYPE.filter(
                                      (item) =>
                                        item.id === option.javaDataType &&
                                        option.parentTableName !== null
                                    ).length > 0
                                      ? FEILD_TYPE.filter(
                                        (item) => item.id === "parentTableName"
                                      ).map((_option, index) => (
                            ...
                            {checked?.includes(option.fieldName) ? (
                              defaultEditable === option.fieldName ? (
                                ...
                                <IconButton
                                  ...
                                  aria-label="default"
                                  onClick={() =>
                                    handleDefaultChange(option.fieldName)
                                  }
                                >
                                  <SettingsIcon
                                    style={{
                                      color:
                                        defaultError.indexOf(
                                          option.fieldName
                                        ) !== -1
                                          ? "#f44336"
                                          : Boolean(
                            ...
                            {checked?.includes(option.fieldName) ? (
                              limitEditable === option.fieldName ? (
                                ...
                                <IconButton
                                  ...
                                  aria-label="limit"
                                  onClick={() =>
                                    handleLimitChange(option.fieldName)
                                  }
                                >
                                  <SettingsIcon
                                    style={{
        ...
        <DialogActions>
          ...
          {featuresAssigned.indexOf(UPDATE_ACTION_APP_SETTINGS) !== -1 && (
            <MatButton type="submit">Update Table</MatButton>
          )}
        </DialogActions>
      </form>
      ...
      {openParentDialog && (
        <ParentTablePopup
          handleClose={closeParentPopup}
          open={openParentDialog}
          handleSave={handleSave}
          childFieldName={childFieldName}
          parentName={payload.filter(
            (item) =>
              item.mapObject?.fieldName ===
              'context')[0]?.parentName}
          parentId={tableDetailsListById.id}
        />
      )}
    </Dialog>
  );
