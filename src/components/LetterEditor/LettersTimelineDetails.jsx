...
const fieldName = {
  ...
  coverSheetName: "Coversheet",
  companyDescription:"Company",
  businessLineDescription:"Lob",
  comment: "Comment",
  letterImportStatus:"Letter Import Status",
  password: "Password",
  ...
  letterImportType:"Letter Type",
  attachmentName: "Letter Attachment"
}; 

const LettersTimelineDetails = (props) => {
  const styles = useStyles();
  const data = formatTimelineData(props.auditDetails, "LETTER");
  ...
  const formatTimelineMessage = (input, value) => {
    let message = "";
    if (input.action === "add")
      message = `${input.createdByUser} created letter`;
    if (input.action === "delete")
      message = `${input.updatedByUser} deleted letter`;
    if (input.action === "update" && input.changes.length > 0) {
      if (value.action === "REPLACE")
        message = (
          <>
            Updated {fieldName[value.key.split("/")[1]]}{" "}
            {(value.key.split("/")[1] === 'termDate' || value.key.split("/")[1] === 'effectiveDate')
              ? <>property from{" "}
              <strong>
                <em>
                  {value.valueFrom ? (
                    formatDateDash(value.valueFrom)
                  ) : (
            ...
            :
            fieldName[value.key.split("/")[1]] !== 'Password' && 
            <>property from{" "}
            <strong>
              <em>
                {value.valueFrom ? (
                    ...
                    <Paper className={styles.paper}>
                      ...
                      {option.changes.length > 0 &&
                        option.changes.map((item, itemkey) => (
                          <List dense={true} className={styles.timelineList} key={itemkey}>
...
export default LettersTimelineDetails;
